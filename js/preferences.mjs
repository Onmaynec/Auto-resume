...
export function addRecentProfile(items, profile, limit = 8, now = Date.now()) {
  ...
  if (!login) return normalizeRecentProfiles(items, limit);
  return normalizeRecentProfiles([
    {
      login,
...
export function removeRecentProfile(items, login) {
  const key = String(login || '').toLowerCase();
  return normalizeRecentProfiles(items).filter((item) => item.login.toLowerCase() !== key);
}

export function readPreferences(storage) {
  try {
    const raw = storage?.getItem?.('auto-resume:preferences:v21');
    if (!raw) return { theme: 'system', recentProfiles: [] };
    const parsed = JSON.parse(raw);
    return {
      theme: normalizeTheme(parsed.theme),
      recentProfiles: normalizeRecentProfiles(parsed.recentProfiles),
    };
  } catch {
    return { theme: 'system', recentProfiles: [] };
  }
}
...
export function writePreferences(storage, preferences) {
  const normalized = {
    ...
    recentProfiles: normalizeRecentProfiles(preferences?.recentProfiles),
  };
  storage?.setItem?.('auto-resume:preferences:v21', JSON.stringify(normalized));
  return normalized;
}
