...
export function applyData(data) {
  ...
  state.languageHistory = normalizeLanguageHistory(data.languageHistory || []);
  ...
  state.selectedProjects = defaultSelection(state.repos);
}
...
async function loadFromProxy(username) {
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 404 && payload.code === 'USER_NOT_FOUND') throw new Error('Пользователь не найден. Проверьте username.');
    ...
    if (error.name === 'AbortError' || error instanceof TypeError) throw new ProxyUnavailableError('Serverless API недоступен.');
    throw error;
  } finally {
...
async function loadDirectFallback(username) {
  ...
  const repos = reposResult.data.filter((repo) => !repo.fork).map((repo) => ({
    ...repo,
...
async function directApi(path) {
  ...
  const rateLimit = {
    ...
    resetAt: Number.isFinite(reset) ? new Date(reset).toISOString() : null,
  };
  if (!response.ok) {
    if (response.status === 404) throw new Error('Пользователь не найден. Проверьте username.');
    if ((response.status === 403 || response.status === 429) && rateLimit.remaining === 0) throw new Error(formatRateLimitMessage(rateLimit.resetAt));
...
function normalizeLanguageHistory(history) {
  return history.map((month) => ({
    key: month.key,
    label: month.label || new Date(`${month.key}-01T00:00:00Z`).toLocaleDateString('ru-RU', { month: 'short' }),
    total: Number(month.total || 0),
    languages: Object.fromEntries(Object.entries(month.languages || {}).map(([name, value]) => [name, {
      count: Number(typeof value === 'object' ? value.count : value),
      color: typeof value === 'object' ? value.color : null,
    }])),
  }));
}
...
function readCache(username) {
  try {
    const key = `auto-resume:v2:${username.toLowerCase()}`;
...
function writeCache(username, data) {
  try {
    localStorage.setItem(`auto-resume:v2:${username.toLowerCase()}`, JSON.stringify({ savedAt: Date.now(), data }));
