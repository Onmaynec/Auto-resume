...
const dictionaries = {
  ru: {
    'meta.title': 'Auto Resume v3.0 — GitHub Resume Generator',
    'meta.description': 'Auto Resume v3.0 — двуязычный PWA-генератор резюме по GitHub с локальными черновиками, сравнением профилей и ATS-экспортом.',
    ...
    'auth.signIn': 'Войти через GitHub', 'auth.signedIn': 'Вход: @{login}', 'auth.notConfigured': 'OAuth доступен после настройки Vercel-переменных',
    'auth.consentTitle': 'Безопасный вход через GitHub',
    'auth.consentBody': 'Auto Resume использует OAuth, чтобы увеличить персональный лимит API и показать вашу собственную приватную статистику вкладов.',
    'auth.permissionProfile': 'read:user: чтение профиля и личной статистики вкладов.',
    'auth.permissionPrivate': 'Приватные и internal вклады учитываются только при анализе вашего аккаунта.',
    'auth.permissionNoCode': 'Доступ к коду, содержимому приватных репозиториев и запись данных не запрашиваются.',
    'auth.securityNote': 'Токен зашифрован в HttpOnly-cookie и недоступен JavaScript, localStorage и публичным ссылкам.',
    'auth.continue': 'Продолжить в GitHub', 'auth.cancel': 'Отмена', 'auth.redirecting': 'Переходим в GitHub…',
    'auth.analyzeSelf': 'Анализировать мой профиль', 'auth.logout': 'Выйти', 'auth.disconnect': 'Отключить доступ',
    'auth.disconnectConfirm': 'Отключить Auto Resume и отозвать выданный GitHub-доступ?',
    'auth.privateEnabled': 'Включена собственная приватная статистика вкладов', 'auth.privateUnavailable': 'read:user не выдан — доступна публичная аналитика',
    'auth.sessionUntil': 'Сессия до {time}',
    ...
    'privacy.microcopy': 'Без входа используются публичные данные. После OAuth-входа read:user добавляет только вашу приватную статистику вкладов; доступ к коду не запрашивается.',
    ...
    'footer': 'GitHub GraphQL API · Auto Resume v3.0 · RU/EN, PWA, локальные черновики и резервные копии',
    ...
    'status.authSuccess': 'GitHub-вход выполнен. Для вашего профиля доступна приватная статистика вкладов.',
    'status.authDenied': 'Авторизация отменена — гостевой режим продолжает работать.', 'status.authError': 'Не удалось завершить GitHub-вход.',
    'status.authUnconfigured': 'GitHub OAuth ещё не настроен на сервере.', 'status.authLogout': 'OAuth-сессия удалена.',
    'status.authDisconnected': 'GitHub-доступ отозван, OAuth-сессия удалена.',
    'status.oauthSelfLoaded': 'Загружена персональная аналитика, включая ограниченные вклады: {count}.',
    'status.oauthPublicLoaded': 'Публичная аналитика загружена через вашу OAuth-сессию.',
    'errors.authLogout': 'Не удалось удалить OAuth-сессию.',
    ...
    'export.creatingDocx': 'Создаём DOCX…', 'export.docxSaved': 'DOCX сохранён ✓', 'export.markdownSaved': 'Markdown сохранён ✓',
    'export.docxSubject': 'Профессиональное резюме', 'export.docxDescription': 'Резюме, созданное локально в Auto Resume по данным GitHub.',
  ...
  en: {
    'meta.title': 'Auto Resume v3.0 — GitHub Resume Generator',
    'meta.description': 'Auto Resume v3.0 — a bilingual PWA resume generator powered by GitHub, with local drafts, profile comparison and ATS export.',
    ...
    'auth.signIn': 'Sign in with GitHub', 'auth.signedIn': 'Signed in: @{login}', 'auth.notConfigured': 'OAuth is available after Vercel environment setup',
    'auth.consentTitle': 'Secure GitHub sign-in',
    'auth.consentBody': 'Auto Resume uses OAuth to increase your personal API limit and include your own private contribution statistics.',
    'auth.permissionProfile': 'read:user: read-only profile and personal contribution statistics.',
    'auth.permissionPrivate': 'Private and internal contributions are included only when analyzing your own account.',
    'auth.permissionNoCode': 'Repository code, private repository contents and write access are never requested.',
    'auth.securityNote': 'The token is encrypted in an HttpOnly cookie and is unavailable to JavaScript, localStorage and public links.',
    'auth.continue': 'Continue to GitHub', 'auth.cancel': 'Cancel', 'auth.redirecting': 'Redirecting to GitHub…',
    'auth.analyzeSelf': 'Analyze my profile', 'auth.logout': 'Sign out', 'auth.disconnect': 'Disconnect access',
    'auth.disconnectConfirm': 'Disconnect Auto Resume and revoke the GitHub authorization?',
    'auth.privateEnabled': 'Private contribution statistics enabled for your account', 'auth.privateUnavailable': 'read:user was not granted — public analytics only',
    'auth.sessionUntil': 'Session until {time}',
    ...
    'privacy.microcopy': 'Guest mode uses public data. After OAuth sign-in, read:user adds only your private contribution statistics; repository code access is never requested.',
    ...
    'footer': 'GitHub GraphQL API · Auto Resume v3.0 · RU/EN, PWA, local drafts and backups',
    ...
    'status.authSuccess': 'GitHub sign-in completed. Private contribution statistics are available for your profile.',
    'status.authDenied': 'Authorization was cancelled — guest mode remains available.', 'status.authError': 'Could not complete GitHub sign-in.',
    'status.authUnconfigured': 'GitHub OAuth is not configured on the server yet.', 'status.authLogout': 'OAuth session removed.',
    'status.authDisconnected': 'GitHub access revoked and the OAuth session removed.',
    'status.oauthSelfLoaded': 'Personal analytics loaded, including restricted contributions: {count}.',
    'status.oauthPublicLoaded': 'Public analytics loaded through your OAuth session.',
    'errors.authLogout': 'Could not remove the OAuth session.',
    ...
    'export.creatingDocx': 'Creating DOCX…', 'export.docxSaved': 'DOCX saved ✓', 'export.markdownSaved': 'Markdown saved ✓',
    'export.docxSubject': 'Professional resume', 'export.docxDescription': 'A resume generated locally by Auto Resume from GitHub data.',
