...
export function createBackup({ workspace, preferences }) { return JSON.stringify({ type: 'auto-resume-backup', version: WORKSPACE_VERSION, exportedAt: new Date().toISOString(), workspace: normalizeWorkspace(workspace), preferences: normalizePreferences(preferences) }, null, 2); }
export function parseBackup(text) { let payload; try { payload = JSON.parse(String(text || '').replace(/^\uFEFF/, '')); } catch { throw codedError('BACKUP_JSON'); } if (payload?.type !== 'auto-resume-backup') throw codedError('BACKUP_TYPE'); if (Number(payload.version) > WORKSPACE_VERSION) throw codedError('BACKUP_NEWER'); return { workspace: normalizeWorkspace(payload.workspace), preferences: normalizePreferences(payload.preferences) }; }
export function normalizePreferences(value) { const source = value && typeof value === 'object' ? value : {}; const theme = ['system', 'dark', 'light'].includes(source.theme) ? source.theme : 'system'; const locale = normalizeLocale(source.locale); const recentProfiles = Array.isArray(source.recentProfiles) ? source.recentProfiles.filter((item) => item && typeof item.login === 'string').slice(0, 8).map((item) => ({ login: item.login, name: item.name || item.login, avatarUrl: item.avatarUrl || '', lastUsed: Number(item.lastUsed || item.openedAt || 0) })) : []; return { theme, locale, recentProfiles }; }
export function profileCacheKeys(storage, prefix = PROFILE_CACHE_PREFIX) { const keys = []; const length = Number(storage?.length || 0); for (let index = 0; index < length; index += 1) { const key = storage.key(index); if (typeof key === 'string' && key.startsWith(prefix)) keys.push(key); } return keys; }
export function clearProfileCache(storage) { const keys = profileCacheKeys(storage); keys.forEach((key) => storage.removeItem(key)); return keys.length; }
function isValidDate(value) { return typeof value === 'string' && Number.isFinite(Date.parse(value)); }
