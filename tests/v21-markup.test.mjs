...
test('theme, history and comparison controls remain present', () => { for (const id of ['themeSelect', 'recentProfiles', 'clearRecentBtn', 'compareForm', 'compareUsername', 'compareResult']) assert.match(html, new RegExp(`id="${id}"`)); assert.match(html, /data-i18n="skip"/); assert.match(html, /v3\.1/); });
