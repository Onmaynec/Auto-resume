...
test('resume audit remains connected to source checks and offline shell', async () => {
  ...
  assert.match(serviceWorker, new RegExp(`APP_VERSION = '${escaped}'`));
  assert.match(versionSource, new RegExp(`APP_VERSION = '${escaped}'`));
...
test('release documentation continues to describe the resume audit privacy boundary', async () => {
