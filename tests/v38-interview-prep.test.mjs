...
test('v3.8 interview prep is connected to syntax checks and offline shell', async () => {
  ...
  for (const file of ['js/interview-prep.mjs', 'js/interview-prep-ui.mjs']) {
...
test('prep data has its own local schema and stays outside public payloads and APIs', async () => {
  const [engine, ui, workspace, share, api] = await Promise.all([read('js/interview-prep.mjs'), read('js/interview-prep-ui.mjs'), read('js/workspace.mjs'), read('js/share.mjs'), read('api/github.js')]);
