...
test('strict semantic versions normalize and compare', () => {
  assert.deepEqual(parseSemver('v3.1.0'), { major: 3, minor: 1, patch: 0, version: '3.1.0' });
  assert.equal(parseSemver('3.1'), null);
  assert.equal(parseSemver('03.1.0'), null);
  assert.equal(compareSemver('3.1.0', '3.0.9'), 1);
  assert.equal(compareSemver('3.1.0', 'v3.1.0'), 0);
...
test('update offer only appears for a newer stable release', () => {
  assert.equal(shouldOfferUpdate('3.1.0', { version: '3.1.1' }), true);
  assert.equal(shouldOfferUpdate('3.1.0', { version: '3.1.0' }), false);
  assert.equal(shouldOfferUpdate('3.1.0', { version: '3.0.9' }), false);
});
